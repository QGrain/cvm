...
fn docs_use_qgrain_repository_and_initial_version() {
    ...
    assert!(readme.contains("v0.1.2"));
    assert!(readme_cn.contains("v0.1.2"));
...
fn install_script_supports_local_checkout_and_source_fallback() {
    ...
    assert!(install.contains("v0.1.2"));
...
fn release_workflow_assets_match_installer_names() {
    ...
    assert!(release_docs.contains("gpg --verify \"$tmp/$asset.sig\" \"$tmp/$asset\""));
    assert!(release_docs.contains("installed cvm matches signed release asset"));
    assert!(release_docs.contains("cmp \"$tmp/cvm\""));
