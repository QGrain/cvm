...
fn version_and_help_do_not_expose_removed_kernel_or_source_flags() {
    ...
    assert_eq!(String::from_utf8_lossy(&version.stdout).trim(), "cvm 0.1.2");
...
fn version_checks_latest_release_without_failing_on_network_success() {
  ...
  "compilers": {"gcc": [], "llvm": []}
    ...
    assert!(output.status.success());
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("cvm 0.1.2"));
    assert!(stdout.contains("new version available: v0.1.3"));
...
fn upgrade_dry_run_uses_remote_index_latest_when_version_is_omitted() {
  ...
  "compilers": {"gcc": [], "llvm": []}
    ...
    assert!(output.status.success());
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("upgrade: v0.1.2"));
    assert!(stdout
        .contains("installer: https://raw.githubusercontent.com/QGrain/cvm/v0.1.2/install.sh"));
}
